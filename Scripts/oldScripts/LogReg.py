...
#46 legendary pokemons out of 721 pokemons
N, M = X.shape
...
Baseline = np.zeros(y_test.shape,dtype=bool)
...
# Fit regularized logistic regression model to training data to predict isLegendary
lambda_interval = np.logspace(-8, 2, 10)
train_error_rate = np.zeros(len(lambda_interval))
test_error_rate = np.zeros(len(lambda_interval))
Baseline_error = np.zeros(len(lambda_interval))
coefficient_norm = np.zeros(len(lambda_interval))
for k in range(0, len(lambda_interval)):
    mdl = LogisticRegression(penalty='l2', C=1/lambda_interval[k], solver='liblinear' )
    ...
    Baseline_error[k]=np.sum(Baseline != y_test)/len(y_test)
...
plt.tight_layout()
#plt.show()  
plt.savefig("../Figures/ClassificationError.png")  
