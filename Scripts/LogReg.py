...
y = dLogReg.iloc[:,list(dLogReg).index('isLegendary')]
dLogReg=dLogReg.drop('isLegendary',axis=1)
print(dLogReg.columns)
attributeNames=list(dLogReg)
X = dLogReg

classNames = ["Not Legendary", "Is Legendary"]
N, M = X.shape
C = len(classNames)
...
lambda_interval = np.logspace(-8, 5, 100)
